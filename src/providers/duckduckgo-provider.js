...
export default class DuckDuckGoProvider extends BaseProvider {
  constructor(config) {
    super(config);
    this.baseUrl = 'https://duckduckgo.com/?iax=images&ia=images&q=';
  }
  ...
  async search(query, options = {}) {
    ...
    // Get image elements
    this.imgHandles = await this.page.$$('img.tile--img__img');
  ...
  async downloadImages(urls, options = {}) {
    ...
    for (const url of urls) {
      const fileName = `duckduckgo_${path.basename(url).split('?')[0]}`;
      ...
      try {
        ...
        Logger.info(`DuckDuckGo: Downloaded ${url} -> ${outPath}`);
        count++;
      } catch (err) {
        Logger.warn(`DuckDuckGo: Failed to download ${url}: ${err.message}`);
      }
    }
