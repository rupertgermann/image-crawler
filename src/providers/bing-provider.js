...
// Simple Bing Images provider using Playwright
export default class BingProvider extends BaseProvider {
  ...
  async search(query, options = {}) {
    const url = `${this.baseUrl}?q=${encodeURIComponent(query)}&form=HDRSC2&first=1&tsc=ImageBasicHover`;
    ...
    // Get image elements
    this.imgHandles = await this.page.$$('a.iusc img');
  ...
  async downloadImages(urls, options = {}) {
    ...
    for (const url of urls) {
      const fileName = `bing_${path.basename(url).split('?')[0]}`;
      ...
      try {
        ...
        Logger.info(`Bing: Downloaded ${url} -> ${outPath}`);
        count++;
      } catch (err) {
        Logger.warn(`Bing: Failed to download ${url}: ${err.message}`);
      }
    }
